...
class EventManager
  ...
  start: (robot)->
    ...
    robot.respond /event\slist$/, (res)=>
      @respond_on_list(res)

    robot.respond /event\sadd\s(\d{1,2}-\d{1,2} \d{1,2}:\d{1,2})\s(.*)/, (res)=>
      [none, datetime, name] = res.match
      @respond_on_add(res, { datetime, name })

    robot.respond /event\sremove\s(\d{1,2}-\d{1,2} \d{1,2}:\d{1,2})\s(.*)/, (res)=>
      [none, datetime, name] = res.match
  ...
  make_list_message: (events)->
    events
      .map((event)-> "#{moment.tz(event.date, TZ).format('MM-DD HH:mm')} #{event.name}")
      .join('\n')
